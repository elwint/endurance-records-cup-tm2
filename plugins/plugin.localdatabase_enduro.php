...
function ldb_connect($aseco) {
	...
	if (!($check[1] && $check[2] && $check[3] && $check[4])) {
		trigger_error('[LocalDB] Table structure incorrect!  Use DOCS/xaseco2.sql to correct this', E_USER_ERROR);
	}

	// add players 'Continent' column
	$fields = array();
	...
	if (!in_array('Continent', $fields)) {
		$aseco->console("[LocalDB] Add 'players' column 'Continent'...");
		mysql_query("ALTER TABLE players ADD Continent tinyint(3) NOT NULL DEFAULT 0 AFTER NickName");
	}
...
function ldb_playerConnect($aseco, $player) {
	...
	// could not be retrieved
		...
		$query = 'INSERT INTO players
		          (Login, Game, NickName, Continent, Nation, TeamName, UpdatedAt)
		          VALUES
